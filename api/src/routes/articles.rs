...
pub fn post_articles(auth: Auth, new_article: Json<NewArticle>, conn: db::Conn) -> Result<JsonValue, Errors> {
    ...
    let article = db::articles::create(&conn, auth.id, &title, &description, &body, &new_article.tag_list);
